...
- **Flow:** Validates the labeled partitions, acquires the updater lock, recovers its dedicated `/run/update-usb` mount tree and fixed desktop staging directory, unlocks the LUKS root, runs `nixos-install` in prebuild mode by default, verifies Home Manager activation, verifies and atomically publishes the new `nix-store.squashfs`, then unmounts and cleans up.
- **Note:** The completed `nix-store.squashfs` is persistent boot data and must survive interruption cleanup; only `nix-store.squashfs.tmp`, host-side staging, mounts, and mapper state are disposable. Script preflight can auto-enter `nix-shell` when `mksquashfs` is missing. Always pass a worktree path containing `flake.nix` (for example `.../main`), not the repo container root.
