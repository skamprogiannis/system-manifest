...
- **Boot Modes:** The USB keeps the default squashfs+tmpfs hybrid store path as native NixOS `fileSystems`, and also exposes manual `ram-store` and `host-auto-store` specialisations through small initrd preparation units. `ram-store` copies `nix-store.squashfs` into RAM before mounting `/nix/store`, uses USB-root scratch for the writable overlay backing directory, and falls back to the USB-backed lower layer if memory is tight. `host-auto-store` aggressively scans large non-removable host Linux partitions first, then `ntfs`/`ntfs3` and `exfat` fallbacks. Linux host partitions can back both the copied squashfs lower image and writable overlay layer; NTFS/exFAT only back the copied lower image, while overlay writes stay on USB-root scratch. If host-auto setup fails, it falls back to the USB-backed lower layer and USB-root scratch. After boot, use `nixos-usb-store-status` to inspect the selected mode and fallback diagnostics.
