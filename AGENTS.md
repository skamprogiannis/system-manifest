...
- **Boot Modes:** The USB keeps the default squashfs+tmpfs hybrid store path, and also exposes manual `ram-store` and `host-auto-store` specialisations. `ram-store` copies `nix-store.squashfs` into RAM before mounting `/nix/store` and falls back to the USB-backed lower layer if memory is tight. `host-auto-store` aggressively scans large non-removable host Linux partitions, copies `nix-store.squashfs` to `.nixos-usb/store`, uses that host copy as the lower store, and uses the same partition for the writable overlay upper/work directories. If host-auto setup fails, it falls back to the default USB-backed lower layer with tmpfs upper.
