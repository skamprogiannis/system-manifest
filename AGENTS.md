...
- **Boot Modes:** The USB keeps the default squashfs+tmpfs hybrid store path as native NixOS `fileSystems`, and also exposes manual `ram-store` and `host-auto-store` specialisations through small initrd preparation units. `ram-store` copies `nix-store.squashfs` into RAM before mounting `/nix/store`, uses USB-root scratch for the writable overlay backing directory, and falls back to the USB-backed lower layer if memory is tight. `host-auto-store` aggressively scans large non-removable host Linux partitions, copies `nix-store.squashfs` to `.nixos-usb/store`, uses that host copy as the lower store, and bind-mounts `.nixos-usb/store/rw` as the writable overlay backing directory. If host-auto setup fails, it falls back to the USB-backed lower layer and USB-root scratch.
