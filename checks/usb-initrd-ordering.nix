...
in {
  usb-initrd-ordering =
    pkgs.runCommand "usb-initrd-ordering-check" {
      nativeBuildInputs = [
        ...
        pkgs.zstd
            ...
            assert_host_auto_units() {
              ...
              assert_contains "RemainAfterExit=true" "$prep_unit" "host-auto prepare service"
              ...
              assert_contains "existing_host_scratch_is_valid" ${usbHostAutoStorePrepareScript} "host-auto prep script"
              assert_contains "active_host_scratch_exists" ${usbHostAutoStorePrepareScript} "host-auto prep script"
              ...
              assert_contains "reused_existing_host_scratch" ${usbHostAutoStorePrepareScript} "host-auto prep script"
            }

            assert_host_auto_reuses_active_session() {
              local test_root="$TMPDIR/host-auto-reuse"
              local host_store="$test_root/host-store"
              local host_scratch="$test_root/host-scratch"
              local rw_store="$test_root/host-store-rw"
              local runtime="$test_root/run"
              local mapper="$test_root/mapper"
              local boot_id="current-boot"
              local active_image="$host_store/.nixos-usb/session/$boot_id/scratch.img"
              local stale_marker="$host_store/.nixos-usb/session/stale-boot/keep"

              mkdir -p \
                "$(dirname "$active_image")" \
                "$(dirname "$stale_marker")" \
                "$host_scratch/store/rw/store" \
                "$host_scratch/store/rw/work" \
                "$rw_store" \
                "$runtime" \
                "$test_root/bin"
              touch \
                "$test_root/nix-store.squashfs" \
                "$stale_marker" \
                "$mapper" \
                "$host_scratch/store/nix-store.squashfs"
              printf '%s\n' encrypted-image-sentinel > "$active_image"
      ...
      [ "$1" = "-q" ]
      case "$2" in
        "$NIXOS_USB_TEST_HOST_STORE"|"$NIXOS_USB_TEST_HOST_SCRATCH"|"$NIXOS_USB_TEST_RW_STORE")
          exit 0
          ;;
      esac
      exit 1
      EOF
              chmod +x "$test_root/bin/mountpoint"

              run_prepare() {
                env \
                  NIXOS_USB_HOST_AUTO_RUNTIME_DIR="$runtime" \
                  NIXOS_USB_HOST_AUTO_LOWER_STORE_IMAGE="$test_root/nix-store.squashfs" \
                  NIXOS_USB_HOST_AUTO_HOST_STORE_MOUNT="$host_store" \
                  NIXOS_USB_HOST_AUTO_HOST_SCRATCH_MOUNT="$host_scratch" \
                  NIXOS_USB_HOST_AUTO_RW_STORE_ROOT="$rw_store" \
                  NIXOS_USB_HOST_AUTO_MAPPER_DEVICE="$mapper" \
                  NIXOS_USB_HOST_AUTO_MOUNTPOINT="$test_root/bin/mountpoint" \
                  NIXOS_USB_HOST_AUTO_MKDIR="${pkgs.coreutils}/bin/mkdir" \
                  NIXOS_USB_HOST_AUTO_BOOT_ID="$boot_id" \
                  NIXOS_USB_TEST_HOST_STORE="$host_store" \
                  NIXOS_USB_TEST_HOST_SCRATCH="$host_scratch" \
                  NIXOS_USB_TEST_RW_STORE="$rw_store" \
                  ${pkgs.bash}/bin/bash ${usbHostAutoStorePrepareScript}
              }
              ...
              if [ ! -f "$active_image" ]; then
                echo "Repeated host-auto preparation deleted the active encrypted scratch image." >&2
                exit 1
              fi
              if ! grep -Fxq encrypted-image-sentinel "$active_image"; then
                echo "Repeated host-auto preparation rewrote the active encrypted scratch image." >&2
                exit 1
              fi
              if [ ! -f "$stale_marker" ]; then
                echo "Repeated host-auto preparation pruned sessions while active scratch was mounted." >&2
                exit 1
              fi
              assert_contains "reused_existing_host_scratch=true" "$runtime/nixos-usb-store-diagnostics" "host-auto reuse diagnostics"
              assert_contains "writable-encrypted-host-auto-overlay" "$runtime/nixos-usb-store-mode" "host-auto reused store mode"
              if [ "$(grep -Fc "reused_existing_host_scratch=true" "$runtime/nixos-usb-store-diagnostics")" -ne 2 ]; then
                echo "Expected both repeated preparations to report active-session reuse." >&2
                exit 1
              fi

              rm "$host_scratch/store/nix-store.squashfs"
              if run_prepare 2>"$test_root/incomplete.err"; then
                echo "Expected incomplete active host scratch to fail closed." >&2
                exit 1
              fi
              if ! grep -Fxq encrypted-image-sentinel "$active_image"; then
                echo "Incomplete active host scratch was destructively replaced." >&2
                exit 1
              fi
              assert_contains "active_scratch_state=invalid" "$runtime/nixos-usb-store-diagnostics" "incomplete host-auto diagnostics"
            }
