...
in {
  hyprland-keybinds =
    pkgs.runCommand "hyprland-keybind-checks" {
      ...
      assert_contains '["name"] = "ghostty-terminal-glass"'
      assert_contains '["class"] = "^(com\\.mitchellh\\.ghostty)$"'
      assert_contains '["no_blur"] = true'
      assert_contains '["tag"] = "+hyprglass_preset_ghostty_terminal"'
      ...
      assert_not_contains '+hyprglass_disabled'
      ...
      assert_file_contains "$hyprglass_helper" 'hg.preset("name:ghostty_terminal'
      assert_file_contains "$hyprglass_helper" 'inherits:subtle'
      assert_file_contains "$hyprglass_helper" 'blur_strength:0.35'
      assert_file_contains "$hyprglass_helper" 'blur_iterations:1'
      assert_file_contains "$hyprglass_helper" 'refraction_strength:0.005'
      assert_file_contains "$hyprglass_helper" 'lens_distortion:0'
      assert_file_contains "$hyprglass_helper" 'chromatic_aberration:0'
      assert_file_contains "$hyprglass_helper" 'adaptive_boost:0'
      assert_file_contains "$hyprglass_helper" 'adaptive_dim:0'
      assert_file_contains "$hyprglass_helper" 'tint_color:0x080a0f16'
      assert_file_contains "$hyprglass_helper" 'contrast:1.20'
      assert_file_contains "$hyprglass_helper" 'saturation:1.06'
      assert_file_contains "$hyprglass_helper" 'vibrancy:0'
      assert_file_not_contains "$hyprglass_helper" 'refraction_strength:0.06'
      assert_file_not_contains "$hyprglass_helper" 'saturation:1.02'
