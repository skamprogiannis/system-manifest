...
- **Desktop local dictation:** Hold `Super+T` to record English speech and release to copy its local Whisper transcription to the clipboard. Notes uses `Super+N`; alerts use `Super+A`. The desktop-only service runs on demand, refuses to run while Bannerlord is open, and removes its transient recording state when it stops.
