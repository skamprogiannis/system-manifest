...
- **Media & Productivity:**
  ...
  - **Vesktop:** Discord client with declarative `Transluence Matugen` theme generation: keeps Transluence UI, removes stock image background, enables true desktop transparency, adds stronger settings blur/readability, and maps translucent colors from DMS' cached Matugen palette without letting DMS rewrite extra CSS files in Vesktop's watched theme folder.
