...
- **Glassmorphism Aesthetics:** Ghostty uses `background-opacity = 0.40` (native RGBA) and applies it to colored cells too, so the terminal background and Codex color blocks stay translucent while text remains fully opaque without overexposing the wallpaper. GTK4 popover styling also softens the Ghostty context menu with a lighter outer border and more translucency. Ghostty disables Hyprland's normal transparent-window blur and uses a low-color-bleed hyprglass terminal preset instead, keeping glass blur without stacked pastel smearing on large surfaces.
