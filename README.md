...
- **Theming:** `modules/home/catppuccin.nix` centralizes the static Mocha palette for Brave, Bat, Ghostty, Zellij, Nixvim, and GTK 3. For wallpaper-driven colours, skwd-wall v2 remains the single Matugen authority: it publishes the full Material palette to DMS, Vesktop regenerates Translucence/QuickCSS directly from DMS `dms-colors.json`, and Hyprland updates window/group borders from that same live palette with a targeted Lua `hl.config(...)` evaluation rather than a full reload.
- **Wallpaper Integration:** skwd-wall v2 is installed through its upstream Nix module and runs `skwd-walld` as a user service. `Super+W` launches `skwd-wall-v2`; the retired v1 Quickshell daemon and custom keybind layer are no longer active. The DMS greeter consumes wallpaper paths from DMS `session.json`, while DMS runtime colours follow the palette published by skwd-wall v2.
