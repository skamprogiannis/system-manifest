...
- **Bannerlord local speech:** `bannerlord-speech start|stop|status` manages on-demand English dictation and NPC speech locally, using CPU Whisper and Kokoro alongside the Codex text adapter. Fourteen English voices share one model, prepared silently at session start. Recording requires a session token and explicit push-to-talk; no audio is sent to Player2. See [speech configuration](modules/home/bannerlord-speech/README.md).
