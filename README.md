...
- **Dual Desktop Environment:**
  ...
  - **Hyprland:** Tiling window manager with glassmorphism aesthetics.
...
- **Media & Productivity:**
  ...
  - **Mailspring:** Email client using GNOME Keyring for credentials.
...
- **Glassmorphism Aesthetics:** High-end transparent UI with premium blur, vibrancy, and dynamic colour-matched borders via Hyprland. Terminals and Nautilus use `opacity 0.82`; browsers and media players are fully opaque.
- **Dynamic Theming:** Matugen-powered Hyprland borders and video wallpapers (`mpvpaper`) synced via a custom `wallpaper-hook` daemon.
- **Zellij Navigation:** `Alt`-based keybindings for all multiplexer actions (pane, tab, scroll, resize), freeing standard `Ctrl` shortcuts for applications. `Alt+s` enters Scroll mode; `Alt+,`/`.` moves tabs.
