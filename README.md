...
- **Bannerlord Codex experiment:** `bannerlord-codex start|stop|status` manages a separate, on-demand text adapter on `127.0.0.1:11435` using the existing Codex ChatGPT sign-in. It uses the shared Codex allowance, starts no requests at login, and records only usage metadata under `~/.local/state/bannerlord-codex/`. Readiness checks do not verify account quota; Player2 remains available independently. See [adapter configuration](modules/home/bannerlord-codex/README.md).
- **Bannerlord local speech:** `bannerlord-speech start|stop|status` manages on-demand English dictation and NPC speech locally, using CPU Whisper and Kokoro alongside the Codex text adapter. Fourteen English voices share one model, prepared silently at session start. Short cancellable phrases and a bounded private audio cache reduce dialogue waits. Recording requires a session token and explicit push-to-talk; no audio is sent to Player2. See [speech configuration](modules/home/bannerlord-speech/README.md).
- **Desktop local dictation:** Hold `Super+Shift+T` to record English speech and release to copy its local Whisper transcription to the clipboard. `Super+T` remains Notes. The desktop-only service runs on demand, refuses to run while Bannerlord is open, and removes its transient recording state when it stops.
- **Bannerlord AI Influence:** The desktop has an on-demand native Player2 Web API bridge, managed by `player2-bannerlord start|stop|status|login`. Browser authorization stays personal; credentials live in `~/.local/state/player2-ai-influence/credentials.json` and never enter the Nix store. The bridge supports cloud requests from Proton without running the Windows Player2 desktop app; desktop microphone capture and local audio playback are unavailable.
- **Media & Productivity:**
