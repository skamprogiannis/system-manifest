...
- **Vesktop:** Discord client with declarative `Translucence + Matugen` theming: generates a canonical `Translucence.theme.css` from DMS' cached Matugen palette, atomically replaces it to reduce live wallpaper-change flicker, keeps the stock Vesktop launcher icon, removes the stock image background, and uses stronger blur/readability tuning over true desktop transparency.
