...
- **Glassmorphism Aesthetics:** Ghostty uses `background-opacity = 0.60` (native RGBA) and applies it to colored cells too, so the terminal background and Codex color blocks stay translucent while text remains fully opaque without overexposing the wallpaper. GTK4 popover styling also softens the Ghostty context menu with a lighter outer border and more translucency. The hyprglass Hyprland plugin is active for lower-strength blur/tint/refraction effects on transparent surfaces.
